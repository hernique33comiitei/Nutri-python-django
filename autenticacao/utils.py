...
def password_is_valid(request, password, confirm_password, email):
    if len(password) < 6:
    ...
    if not re.search('[1-9]', password):
        ...
        return False
    if len(password) == 0:
        messages.add_message(request, constants.ERROR, 'Sua senha está vazia')
        return False
    if len(email) == 0:
        messages.add_message(request, constants.ERROR, 'Seu email está vazio')
        return False
